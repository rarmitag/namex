...
OUTPUT=$($KCADM create clients -r master -s clientId=$1 -s enabled=true -s 'redirectUris=["http://0.0.0.0:8080/*", "http://localhost:8080/*"]' --no-config --server http://localhost:8080/auth --realm master --user admin --password admin 2>&1)
